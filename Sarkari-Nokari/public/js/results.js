...
function getCategoryFromText(text) {
  ...
  if (/(^|\\s)(up|bihar|mp|rajasthan|punjab|haryana|gujarat|maharashtra)(\\s|$)/.test(t))
...
function showActiveStateFilter() {
  ...
  const stateNames = {
    ...
    py: "Puducherry",       central: "Central Govt / All India",
  };
  ...
  banner.innerHTML = `
    ...
    <a href="/nokari/result" class="state-filter-clear">✕ Clear</a>
  `;

  const header = document.querySelector(".page-header");
  if (header) header.insertAdjacentElement("afterend", banner);

  document.title = `Results – ${label} | Sarkari Naukari`;
}
...
function setupCategoryFilters() {
  ...
  function filterCards() {
    ...
    // TOP INDEX LIST
    const indexItems = document.querySelectorAll(".result-index-item");
